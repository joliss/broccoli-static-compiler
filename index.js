...
StaticCompiler.prototype.rebuild = function () {
  var self = this
    ...
    var sourcePath = path.join(this.inputPath, self.options.srcDir)
    var destPath   = path.join(this.outputPath, self.options.destDir)

    if (self.options.files == null) {
      self._copy(sourcePath, destPath)
    } else {
      var baseDir = path.join(this.inputPath, self.options.srcDir)
